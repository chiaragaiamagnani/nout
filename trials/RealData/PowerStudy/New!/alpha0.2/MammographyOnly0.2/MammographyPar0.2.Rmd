---
title: "Power analysis on Mammography dataset"
...
The aim is to compare on Mammography datasets the performance of three closed testing procedures, which respectively use Simes local test with and without Storey estimator for the proportion of true null hypotheses and Wilcoxon-Mann-Whitney local test.
...
## Mammography dataset
Mammography dataset (available at http://odds.cs.stonybrook.edu/mammography-dataset/) consists of $11183$ observations, among which $n_{inliers}=10923$ items are inliers and the remaining $n_{outliers}=260$ are outliers.
...
In the case of Mammography dataset we obtain $l=3463, \hspace{2mm} m=1999, \hspace{2mm} n=400$.
...
boxplot(results$discoveries, main="Mammography | Distribution of the number of discoveries")
...
resMammo0 = results
save(resMammo0,
     file="~/nout/trials/RealData/PowerStudy/New!/alpha0.2/MammographyOnly0.2/resMammo0")
...
### 10% outliers
We now set the proportion of inliers equal to $0.9$. Referring to Digits dataset we have that the number of inliers is $n_0=360$ and the number of outliers is $n_1=40$.
...
resMammo10 = results
save(resMammo10,
     file="~/nout/trials/RealData/PowerStudy/New!/alpha0.2/MammographyOnly0.2/resMammo10")
