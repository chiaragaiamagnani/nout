...
The aim is to compare on ALOI datasets the performance of three closed testing procedures, which respectively use Simes local test with and without Storey estimator for the proportion of true null hypotheses and Wilcoxon-Mann-Whitney local test.
...
## ALOI dataset
Covertype dataset (available at https://www.dbs.ifi.lmu.de/research/
outlier-evaluation/DAMI/literature/ALOI) consists of $49534$ observations, among which $n_{inliers}=48026$ items are inliers and the remaining $n_{outliers}=1508$ are outliers.
...
In the case of ALOI dataset we obtain $l=14014, \hspace{2mm} m=9999, \hspace{2mm} n=2000$.
...
boxplot(results$discoveries, main="ALOI | Distribution of the number of discoveries")
...
resALOI0 = results
save(resALOI0,
     file="~/nout/trials/RealData/Datasets/Dataset ALOI/resALOI0")
...
resALOI10 = results
save(resALOI10,
     file="~/nout/trials/RealData/Datasets/Dataset ALOI/resALOI10")
