             ...
             function(m1){
               foreach(b = 1:B, .combine=cbind) %dopar% {
                ...
                if(m1!=0){
                  ...
                  in_index3 = setdiff(in_index2, cal_ind)
                  tein_ind = sample(in_index3, size = m0)
                  teoutaug_ind = sample(out_ind, size = (k*m1))
                  ...
                  d_WMW = d_MannWhitney(S_X=S_cal, S_Y=S_te, alpha=alpha)
                  d_WMWk3 = d_MannWhitneyk3(S_X=S_cal, S_Y=S_te, alpha=alpha)
                  d_Simes = d_Simes(S_X=S_cal, S_Y=S_te, alpha=alpha)
                  d_StoSimes = d_StoreySimes(S_X=S_cal, S_Y=S_te, alpha=alpha)
                  d_BH = d_benjhoch(S_X=S_cal, S_Y=S_te, alpha=alpha)
                  d_StoBH = d_StoreyBH(S_X=S_cal, S_Y=S_te, alpha=alpha)
                }
...
resCreditCard = res
save(resCreditCard, file="~/nout/trials/RealData/PowerStudy/New!/alpha0.2/CreditCardOnly0.2/resCreditCardk3")
