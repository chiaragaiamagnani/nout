...
scores_from_mixture = function(k, raw_scores, m1){
  m = length(raw_scores)
  scores = rep(0, times = m)

  if(m1==0)  scores = raw_scores

  if(m1==m){
    scores = sapply(0:(m1-1), function(i) max(usable.raw_scores[(i*k+1):(i*k+k)]))
  }

  if(0<m1 & m1<m){
    scores.out = sapply(0:(m1-1), function(i) max(usable.raw_scores[(i*k+1):(i*k+k)]))
    scores.in = raw_scores[(k*m1+1):m] # qua errore!!
...
simuLMPI = function(B=10^4, n, l, m, d = 3, k = 2, theta, alpha = m/(l+1)){
  ...
  for(b in 1:B){
    ...
    S_te = scores_from_mixture(k=k, raw_scores=rawS_te, m1=m1)
