...
calc.theory.var.T4 <- function(m,n) {
  N <- n+m
  lambda <- m/N
  z_10 <- (10826*lambda^4-8019*lambda^3+51651)/(35*lambda^4*(1-lambda)^4)
  z_01 <- (77868*lambda^4-377712*lambda^3+765144*lambda^2-1201050*lambda+287505)/(35*lambda^4*(1-lambda)^4)
  var <- 9/N*(z_10/lambda+z_01/(1-lambda))
  return(var)
}
...
for(id1 in 1:L1) {
  ...
  for(id2 in 1:L2) {
    ...
    #mean.theory <- calc.theory.mean.T4(m,n)
    variance.theory <- calc.theory.var.T4(m,n)
    n.list.out[id] <- n
    lambda.list[id] <- lambda
    # empirical.mean.list[id] <- mean(stats)
    # theoretical.mean.list[id] <- mean.theory
