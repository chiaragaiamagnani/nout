...
B=5000; B_MC=500 # number of repetitions
...
X = rg_null(x=150) #4000
X1 = X[1:75] # 2000
X2 = X[76:150] #2001:4000
                 ...
                 res2 = sapply(1:length(thetas),
                               function(j){
                                 ...
                                 res1 = sim_pow(B=B, B_MC=B_MC, m=sizes[s], n=sizes[s], theta=thetas[j],
                                                ...
                                                g_hat=g_KDE[[ll]], g_hat_monotone=g_mono[[ll]],
                                                alpha=alpha)
...
for(i in 1:length(g_alt_list)){
  ...
  g_mono[[i]] = estimate_g(X1=X1, X2=X2, Y=Y, constraint=constr[i], ker="uniform")
  g_KDE[[i]] = estimate_g(X1=X1, X2=X2, Y=Y, ker="uniform")
}
toc()
